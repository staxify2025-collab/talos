...
export async function visionMap(request: VisionMapRequest): Promise<VisionMapResponse> {
  const {
    tenantId,
    schemaId,
  ...
  try {
    ...
    const mappings = schema.mappings as Array<{
      sourceField: string;
      targetLabel: string;
...
async function logJobRun(
  ...
  success: boolean,
  executionPath: 'WEBMCP' | 'VISION' = 'VISION'
): Promise<void> {
  try {
    const runRef = db
      .collection('tenants')
    ...
    console.log(
      `[Perf Log] tenant=${tenantId} path=${executionPath} cache=${cacheStatus} ` +
 ...
 */
export async function logWebMCPRun(
  tenantId: string,
  schemaId: string,
  success: boolean
): Promise<void> {
