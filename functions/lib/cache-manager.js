...
function getClient() {
    if (!_client) {
        _client = new genai_1.GoogleGenAI({
            vertexai: true,
            project: PROJECT_ID,
            location: LOCATION,
        });
 ...
 */
async function getOrCreateCache(tenantId, schemaId, targetScreenshotBase64, systemInstruction, mappingDescription) {
    const cacheRef = db
        .collection('tenants')
        .doc(tenantId)
    ...
    const now = admin.firestore.Timestamp.now();
    const expiresAt = admin.firestore.Timestamp.fromMillis(now.toMillis() + 3600 * 1000 // 1 hour
    );
    const metadata = {
 ...
 */
async function invalidateCache(tenantId, schemaId) {
    const client = getClient();
    if (schemaId) {
